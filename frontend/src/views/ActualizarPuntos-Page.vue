<template>
  <div class="container">
    ...
    <form @submit.prevent="editarPuntoDeControl" class="checkpoint-form">
      ...
      <button type="submit" class="btn-submit">Actualizar Punto de Control</button>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    async editarPuntoDeControl() {
      try {
        const response = await axios.put('http://localhost:8080/actualizar_punto_de_control', this.puntoDeControl);
        ...
        console.error(error);
        alert('Error al actualizar el punto de control');
      }
    },
    limpiarCampos() {
      this.puntoDeControl = {
